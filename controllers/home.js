    ...
    if (email == "admin@gmail.com" && pass == 0000) {
        ...
        ClinicalEngineer.findOne({ where: { Email: email } }).then(
            (clinicalengineer) => {
                if (clinicalengineer) {
                    bcrypt
                        .compare(pass, clinicalengineer.Password)
                        .then((result) => {
                            if (result) {
...
exports.dialyInspectionEngineerPost = (req, res) => {
    ...
    Equipment.findByPk(equipmentId)
        .then((equipment) => {
            if (equipment) {
                ClinicalEngineer.findByPk(engineerId).then(
                    (clinicalengineer) => {
                        if (clinicalengineer) {
                            ...
                            res.render("error", {
                                layout: false,
                                pageTitle: "Error",
                                href: "/engineer/dialyInspection",
                                message:
                                    "Sorry !!! Could Not Get this Engineer",
                            })
                        }
                    }
                )
        ...
        .catch((err) => {
            if (err) {
                ...
                res.render("error", {
                    ...
                    pageTitle: "Error",
                    href: "/home",
                    message: "Sorry !!! Could Not Get Departments",
                })
            }
        })
}
...
exports.clinicalEngineer = (req, res) => {
    ClinicalEngineer.findAll({ include: [{ model: Department }] })
        .then((clinicalEngineers) => {
            ...
            res.render("clinicalEngineer", {
                pageTitle: "clinicalEngineer",
                CE: true,
                clinicalEngineers: clinicalengineers,
                hasEngineers: clinicalengineers.length > 0,
            })
        })
        .catch((err) => {
            if (err)
        ...
        .catch((err) => {
            if (err)
                res.render("error", {
                    ...
                    pageTitle: "Error",
                    href: "/home",
                    message: "Sorry !!! Could Not Get Agents",
                })
        })
}
    ...
    WorkOrder.findAll({
        ...
        .then((workorders) => {
            const wd = workorders.map((workD) => {
                return {
                    ...
                    ClinicalEnginner:
                        workD.ClinicalEnginner.FName +
                        " " +
                        workD.ClinicalEnginner.LName,
        ...
        .catch((err) => {
            ...
            res.render("error", {
                ...
                pageTitle: "Error",
                href: "/home",
                message: "Sorry !!! Could Not Get WorkOrders",
            })
        })
}
        ...
        .catch((err) => {
            if (err)
                res.render("error", {
                    ...
                    pageTitle: "Error",
                    href: "/home",
                    message: "Sorry !!! Could Not Get BreakDowns",
                })
        })
}
    ...
    DailyInspection.findAll({
        ...
        .catch((err) => {
            res.render("error", {
                ...
                pageTitle: "Error",
                href: "/",
                message: "Sorry !!! Could Not Get Report",
            })
        })
}
    ...
    WorkOrder.findAll({ where: { ClinicalEnginnerDSSN: dssn } })
            ...
            var events = orders.map((order) => {
                return {
                    ...
                    start:
                        ...
                        "-" +
                        order.StartDate.split("-")[2] +
                        " " +
                        "00:00:00Z",
                    end:
                        order.EndDate.split("-")[0] +
                        "-" +
                        order.EndDate.split("-")[1] +
                        "-" +
                        order.EndDate.split("-")[2] +
                        " " +
                        "23:00:00Z",
        ...
        .catch((err) => {
            res.render("error", {
                ...
                pageTitle: "Error",
                href: "/",
                message: "Sorry !!! Could Not Get Orders",
            })
        })
}
