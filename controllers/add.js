...
exports.addDepartment = (req, res) => {
    ...
    Department.create({ Code: code, Name: name, Location: location })
        .then((dep) => {
...
exports.addEquipment = (req, res) => {
    ...
    Department.findOne({ where: { Name: department } })
        .then((department) => {
            if (department) {
                departmentCode = department.Code
                AgentSupplier.findOne({ where: { Id: agent } }).then(
                    (agent) => {
                        if (agent) {
                            agentCode = agent.Id
                            ...
                            res.render("error", {
                                ...
                                pageTitle: "Error",
                                href: "/equipment",
                                message: "Sorry !!! Could Not Get this Agent",
                            })
                    }
                )
            } else {
        ...
        .catch((err) => {
            if (err)
                res.render("error", {
                    ...
                    pageTitle: "Error",
                    href: "/sparePart",
                    message: "Sorry !!! Could Not Add This Engineer ",
                })
        })
}
    ...
    AgentSupplier.findOne({ where: { Id: agentId } })
        ...
        .catch((err) => {
            res.render("error", {
                ...
                pageTitle: "Error",
                href: "/sparePart",
                message: "Sorry !!! Could Not Gey rhis page",
            })
        })
}
...
exports.addWorkOrder = (req, res) => {
    ...
    var equId = null
    var engId = null
    Equipment.findOne({ where: { Code: equipmentId } })
        .then((equipment) => {
            if (equipment) {
                equId = equipment.Code
                ClinicalEngineer.findOne({ where: { DSSN: engineerId } }).then(
                    (clinicalengineer) => {
                        if (clinicalengineer) {
        ...
        .catch((err) => {
            if (err)
                res.render("error", {
                    ...
                    pageTitle: "Error",
                    href: "/workOrder",
                    message: "Sorry !!! Could Not Add This Work Order ",
                })
        })
}
...
exports.addMaintenance = (req, res) => {
    ...
    description = req.body.Description
    var breakdown = null
    BreakDowns.findOne({ where: { Code: breakdowncode } })
        .then((breakdown) => {
            if (breakdown) {
                ...
                return res.render("error", {
                    ...
                    pageTitle: "Error",
                    href: "/maintenance",
                    message: "Sorry !!! Could Not Get this Break down",
                })
            console.log(err)
        })
        .catch((err) => {
